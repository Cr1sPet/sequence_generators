...
# -----------------------
# Генерация распределения Парето
# -----------------------
def pareto_random(x_min, c, n = 10000)
  n.times.map { x_min / ((1 - rand) ** (1.0 / c)) }
end

def pareto_pdf(x, x_min, c)
  return 0 if x < x_min
  c * (x_min ** c) / (x ** (c + 1))
end
...
# -----------------------
# Улучшенные тесты для распределения Парето
# -----------------------
def frequency_test_pareto(data, x_min, c, n_bins = 20)
  # Для Парето используем логарифмические интервалы
  min_val = x_min
  max_val = data.max
  log_min = Math.log(min_val)
  log_max = Math.log(max_val)
  log_bin_width = (log_max - log_min) / n_bins
  ...
  data.each do |value|
    if value <= max_val
      ...
      bin_index = ((log_val - log_min) / log_bin_width).to_i
      bin_index = [bin_index, n_bins - 1].min
      counts[bin_index] += 1
    else
      counts[n_bins] += 1
    end
  ...
  # Расчет ожидаемых частот для распределения Парето
  expected = []
  n_bins.times do |i|
    x_low = Math.exp(log_min + i * log_bin_width)
    x_high = Math.exp(log_min + (i + 1) * log_bin_width)
    # Вероятность попадания в интервал [x_low, x_high]
    prob = (x_min / x_low) ** c - (x_min / x_high) ** c
    expected << data.size * prob
  end
  # Хвост (значения > max_val)
  prob_tail = (x_min / max_val) ** c
...
def correlation_test_pareto(data, n_lags = 5)
  results = {}
...
def runs_test_pareto(data, n_thresholds = 3)
  results = {}
...
# -----------------------
def run_comprehensive_pareto_tests
  test_configs = [
    {x_min: 1.0, c: 1.5, n_samples: 5000, name: "x_min=1.0, c=1.5 (тяжелый хвост)"},
    {x_min: 1.0, c: 2.0, n_samples: 10000, name: "x_min=1.0, c=2.0 (стандартный)"},
    {x_min: 1.0, c: 3.0, n_samples: 15000, name: "x_min=1.0, c=3.0 (легкий хвост)"},
    {x_min: 2.0, c: 2.0, n_samples: 8000, name: "x_min=2.0, c=2.0 (сдвинутый)"}
  ]
  ...
  test_configs.each do |config|
    ...
    # Генерация данных
    data = pareto_random(config[:x_min], config[:c], config[:n_samples])
    
    # Основные статистики
    mean = data.sum / data.size
    variance = data.map { |x| (x - mean) ** 2 }.sum / data.size
    
    # Теоретические моменты (для c > 1 и c > 2 соответственно)
    theoretical_mean = config[:c] > 1 ? config[:c] * config[:x_min] / (config[:c] - 1) : Float::INFINITY
    theoretical_variance = config[:c] > 2 ? (config[:x_min] ** 2 * config[:c]) / ((config[:c] - 1) ** 2 * (config[:c] - 2)) : Float::INFINITY
    
    puts "ОСНОВНЫЕ СТАТИСТИКИ:"
    puts "  Выборочное среднее: #{mean.round(4)}"
    puts "  Теоретическое среднее: #{theoretical_mean.round(4)}" if theoretical_mean.finite?
    puts "  Выборочная дисперсия: #{variance.round(4)}"
    puts "  Теоретическая дисперсия: #{theoretical_variance.round(4)}" if theoretical_variance.finite?
    ...
    [10, 15, 20].each do |bins|
      test_result = frequency_test_pareto(data, config[:x_min], config[:c], bins)
    ...
    puts "-" * 50
    corr_results = correlation_test_pareto(data, 5)
    ...
    puts "-" * 50
    runs_results = runs_test_pareto(data, 3)
    ...
    all_results[config[:name]] = {
      ...
      params: {x_min: config[:x_min], c: config[:c]}
    }
  end
...
# -----------------------
def plot_pareto_results(results, x_min, c)
  data = results[:data]
  
  # Ограничим данные для визуализации (Парето имеет тяжелый хвост)
  max_visual = 10.0
  data_visual = data.select { |x| x <= max_visual }
  
  # Гистограмма с логарифмическими интервалами
  n_bins = 50
  histogram = Array.new(n_bins, 0)
  log_min = Math.log(x_min)
  log_max = Math.log(max_visual)
  log_bin_width = (log_max - log_min) / n_bins

  data_visual.each do |x|
    log_val = Math.log(x)
    bin = ((log_val - log_min) / log_bin_width).to_i
    bin = n_bins - 1 if bin >= n_bins
    histogram[bin] += 1
  end

  # Нормализация гистограммы (переход к линейной шкале)
  total = data_visual.size.to_f
  histogram_normalized = []
  n_bins.times do |i|
    x_low = Math.exp(log_min + i * log_bin_width)
    x_high = Math.exp(log_min + (i + 1) * log_bin_width)
    bin_width_linear = x_high - x_low
    histogram_normalized << histogram[i] / (total * bin_width_linear)
  end

  # Теоретическая кривая
  x_theoretical = (0..1000).map { |i| x_min + i * (max_visual - x_min) / 1000.0 }
  y_theoretical = x_theoretical.map { |x| pareto_pdf(x, x_min, c) }

  # Данные для гистограммы (центры интервалов в линейной шкале)
  x_hist = (0...n_bins).map { |i| Math.exp(log_min + (i + 0.5) * log_bin_width) }
  y_hist = histogram_normalized

  begin
    Numo.gnuplot do
      set title: "Распределение Парето (x_min=#{x_min}, c=#{c})"
      set xlabel: 'x'
      set ylabel: 'Плотность вероятности f(x)'
      set xrange: x_min..max_visual
      set style: 'fill solid 0.5'
      set terminal: 'pngcairo enhanced font "Arial,12"'
      set output: "pareto_xmin_#{x_min}_c_#{c}.png"
...
# -----------------------
puts "=" * 70
puts "КОМПЛЕКСНОЕ ТЕСТИРОВАНИЕ РАСПРЕДЕЛЕНИЯ ПАРЕТО"
puts "=" * 70
puts "Множественное тестирование с разными параметрами x_min и c"
puts

# Запуск всестороннего тестирования
results = run_comprehensive_pareto_tests

# Визуализация результатов
puts "\nПостроение графиков распределения..."
results.each do |name, config|
  params = config[:params]
  plot_pareto_results(config, params[:x_min], params[:c])
