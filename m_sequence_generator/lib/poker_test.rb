...
def poker_test(samples, digits: 5)
  return { counts: {}, expected: {}, chi2: 0.0, df: 0, p_value: 1.0 } if samples.empty?
...
samples.each do |v|
  ...
  # выровнять по ключам
  observed = probs.keys.map { |k| counts[k] || 0 }
  ...
  { counts: counts, expected: expected, chi2: chi2, df: df, p_value: p_value }
end
