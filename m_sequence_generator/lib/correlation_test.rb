...
def correlation_test(samples, lags: [1, 2, 5, 10])
  n = samples.length
  return { mean: 0, var: 0, results: lags.map { |lag| [lag, {rho:0.0, z:0.0, p_value:1.0}] }.to_h } if n == 0

  mean = samples.sum / n.to_f
  var = samples.sum { |x| (x - mean)**2 } / n
  return { mean: mean, var: var, results: lags.map { |lag| [lag, {rho:0.0, z:0.0, p_value:1.0}] }.to_h } if var.zero?
